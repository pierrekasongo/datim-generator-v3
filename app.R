...
server <- function(input, output){
  ...
  dataset <- reactive({
    ...
    last_master <- merge_cc %>% 
      ...
      #select(dataelementuid_datim,orgUnit_datim,AttributeOptionComboID,option_combo_uid_datim,Value) %>%
      rename(dataelementID = dataelementuid_datim, categoryOptionComboID = option_combo_uid_datim,orgUnitID = orgUnit_datim) %>%
      #mutate(Period = period,AttributeOptionComboID = attribute_combo) %>%
